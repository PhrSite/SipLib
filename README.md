# Introduction
This project is a portable, cross-platform class library written in the C# language that targets the .NET 9.0 environment. It may be used by applications that target the Windows Desktop(version 10 or later), Windows Server or Linux operating systems.
...
2. Microsoft.Extensions.Logging (8.0.0)
3. DnsClient (1.8.0)
...
|--------|--------|
| ClassLibrary | Contains the project files for the SipLib project and various subdirectories that contain the source code. See below.  |
| docs | The docs directory contains HTML and other files generated by DOCFX and used for the GitHub pages located at https://phrsite.github.io/SipLib. |
...
| Samples | Contains some sample applications that demonstrate how to use the SipLib class library. |
| Testing | Contains the XUnit unit tests for the SipLib class library. Also contains test projects for testing the AMR-WB audio encoder included in the SipLib class library. |
| Test Programs | Contains a test probram called TestAes256 for testing an audio call using SDES-SRTP with AES-256 encryption. |
