...
/// <summary>
/// Class that receives H264 encoded video Network Access Layer (NALs) in RTP packets and builds complete
/// H264 encoded access units that can then be decoded.
/// </summary>
public class H264Depacketiser
{
    ...
    private uint previous_timestamp = 0;

    // Used for diagnostics stats
    private int norm, fu_a, fu_b, stap_a, stap_b, mtap16, mtap24 = 0;

    // Used to assemble the RTP packets that form one RTP Frame
    private List<KeyValuePair<int, byte[]>> temporary_rtp_payloads = new List<KeyValuePair<int, byte[]>>();

    // Used to concatenate fragmented H264 NALs where NALs are splitted over RTP packets
    private MemoryStream fragmented_nal = new MemoryStream();

    /// <summary>
    /// Processes a single RTP packet containing one or more H264 NALs and builds up a full H264 access unit
    /// frame.
    /// </summary>
    /// <param name="rtpPayload">The payload of the RTP packet</param>
    /// <param name="seqNum">The SEQ field of the RTP packet</param>
    /// <param name="timestamp">The Timestamp field of the RTP packet</param>
    /// <param name="markbit">The marker bit field of the RTP packet</param>
    /// <param name="isKeyFrame">Output. Set to true if the frame is a key frame.</param>
        ...
        if (rtp_marker == 1)
        {
            ...
            if (temporary_rtp_payloads.Count > 1)
            {
                temporary_rtp_payloads.Sort((a, b) => { 
                    // Detect wraparound of sequence to sort packets correctly (Assumption that no more than
                    // 2000 packets per frame)
                    return (Math.Abs(b.Key - a.Key) > (0xFFFF - 2000)) ? -a.Key.CompareTo(b.Key) : 
                    a.Key.CompareTo(b.Key);                        
                    });
            }

            // End Marker is set. Process the list of RTP Packets (forming 1 RTP frame) and save the NALs
            // to a file
    ...
    private List<byte[]> ProcessH264PayloadFrame(List<KeyValuePair<int, byte[]>> rtp_payloads, out bool 
        isKeyFrame)
    {
        ...
        for (int payload_index = 0; payload_index < rtp_payloads.Count; payload_index++)
        {
            ...
            else if (nal_header_type == 24)
            {
                ...
                try
                {
                    ...
                    {
                        int size = (rtp_payloads[payload_index].Value[ptr] << 8) + (rtp_payloads[payload_index].
                            Value[ptr + 1] << 0);
                        ptr = ptr + 2;
            ...
            else if (nal_header_type == 28)
            {
                ...
                if (fu_header_s == 1 && fu_header_e == 0)
                {
                    ...
                    // Build the NAL header with the original F and NRI flags but use the the Type field from
                    // the fu_header_type
                    byte reconstructed_nal_type = (byte)((nal_header_f_bit << 7) + (nal_header_nri << 5) + 
                        fu_header_type);
                    ...
                    fragmented_nal.Write(rtp_payloads[payload_index].Value, 2, rtp_payloads[payload_index].
                        Value.Length - 2);
                }
