...
/// 
/// The SRTPCryptoContext class acts as a manager class and maintains all the information used in SRTP 
/// transformation. It is responsible for deriving encryption keys / salting keys / authentication keys from
...
public class SrtpCryptoContext
{
    ...
    /// <param name="rocIn">the initial Roll-Over-Counter according to RFC 3711. These are the upper 32 bit of the
    /// overall 48 bit SRTP packet index. Refer to chapter 3.2.1 of RFC 3711.</param>
    /// <param name="kdr">the key derivation rate defines when to recompute the SRTP session keys. Refer to 
    /// chapter 4.3.1 in the RFC.</param>
    /// <param name="masterK">byte array holding the master key for this SRTP cryptographic context.
    /// Refer to chapter 3.2.1 of the RFC about the role of the master key.</param>
    ...
    /// <summary>
    /// Gets the Roll-Over-Counter of this SRTP cryptographic context
    /// </summary>
    /// <returns></returns>
    public int GetROC()
    {
    ...
    /// <summary>
    /// Sets the Roll-Over-Counter of this SRTP cryptographic context
    /// </summary>
    /// <param name="rocIn">The Roll-Over-Counter of this SRTP cryptographic context</param>
    ...
    /// <summary>
    /// Transforms a RTP packet into a SRTP packet. This method is called when a normal RTP packet ready to be sent.
    /// 
    /// Operations done by the transformation may include: encryption, using either Counter Mode encryption, or F8
    /// Mode encryption, adding authentication tag, currently HMC SHA1 method.
    /// 
    ...
    /// 
    /// Both encryption and authentication functionality can be turned off as long as the SRTPPolicy used in this
    /// SRTPCryptoContext requires no encryption and no authentication.Then the packet will be sent out 
    /// untouched.However this is not encouraged. If no SRTP feature is enabled, then we shall not use SRTP
    /// TransformConnector. We should use the original method (RTPManager managed transportation) instead.
    /// </summary>
    /// <param name="pkt">The RTP packet that is just received</param>
    /// <returns>Returns true if the packet can be accepted false if the packet failed authentication or failed
    /// replay check</returns>
...
#pragma warning restore CS0675 // Bitwise-or operator used on a sign-extended operand
    ...
    /// <param name="rocIn">Roll-Over-Counter</param>
    ...
    /// This method supports a 64 packet history relative the the given sequence number.
    /// Sequence Number is guaranteed to be real (not faked) through authentication.
    /// </summary>
...
#pragma warning restore CS0675 // Bitwise-or operator used on a sign-extended operand
    ...
    /// <summary>
    /// Computes the initialization vector, used later by encryption algorithms, based on the lable, the packet
    /// index, key derivation rate and master salt key.
    /// </summary>
    /// <param name="label">Label specified for each type of iv</param>
    /// <param name="index">48bit RTP packet index</param>
    ...
    /// </summary>
    /// <param name="seqNo">Sequence number of the received RTP packet</param>
    /// <returns>Returns the new SRTP packet index</returns>
...
#pragma warning restore CS0675 // Bitwise-or operator used on a sign-extended operand
    ...
    /// <summary>
    /// Updates the SRTP packet index.
    /// 
    /// This method is called after all checks were successful. See section 3.3.1 in RFC3711 for detailed 
    /// description.
    /// </summary>
    /// <param name="seqNo">Sequence number of the accepted packet</param>
    /// <param name="guessedIndex">Guessed roc</param>
...
#pragma warning restore CS0675 // Bitwise-or operator used on a sign-extended operand
    ...
    /// <param name="roc">The Roll-Over-Counter for this context</param>
    /// <param name="deriveRate">The key derivation rate for this context</param>
