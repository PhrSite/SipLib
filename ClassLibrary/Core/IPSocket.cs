...
public class IPSocket
{
    ...
    public static string GetSocketString(IPEndPoint endPoint)
    {
        string format = (endPoint.Address.AddressFamily == AddressFamily.InterNetworkV6) ? 
            "[{0}]:{1}" : "{0}:{1}";
        return string.Format(format, endPoint.Address.ToString(), endPoint.Port.ToString(NumberFormatInfo.
            InvariantInfo));
    }
    ...
    /// <param name="result">If the parse is successful this output parameter will contain the IPv4 or IPv6 
    /// end point.</param>
    ...
    public static bool TryParseIPEndPoint(string s, out IPEndPoint result)
    {
        ...
        if (IPAddress.TryParse(s.Substring(0, addressLength), out IPAddress address))
        {
            ...
            if (addressLength == s.Length ||
                (uint.TryParse(s.Substring(addressLength + 1), NumberStyles.None, CultureInfo.
                InvariantCulture, out port) && port <= MaxPort))
            {
    ...
    /// <summary>
    /// Checks the Contact SIP URI host and if it is recognised as a private address it is replaced 
    /// with the socket the SIP message was received on.
    /// 
    ...
    /// </summary>
    /// <remarks>
    /// rj2: I had the requirement of parsing an IPEndpoint with IPv6, v4 and hostnames and getting them 
    /// as string and int
    /// </remarks>
    /// <param name="endpointstring">string to check</param>
    /// <param name="host">host-portion of <paramref name="endpointstring"/>, if host can be parsed as I
    /// PAddress, then <paramref name="host"/> is IPAddress.ToString</param>
    ...
    /// <exception cref="System.ArgumentException">if <paramref name="endpointstring"/> is null/empty 
    /// </exception>
    /// <exception cref="System.FormatException">if host looks like ip-address but can't be 
    /// parsed</exception>
    ...
    {
        if (string.IsNullOrEmpty(endpointstring) == true)
            throw new ArgumentException("Endpoint descriptor must not be empty.");
